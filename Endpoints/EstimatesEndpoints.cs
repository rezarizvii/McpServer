...
{
    public static void MapEstimatesEndpoints(this WebApplication app)
    {
            ...
            IConfiguration config) =>
        {
            ...
            string? nextUrl = $"{apiBase}/estimates?{queryParams}";
            string? lastUrl = null;
            ...
            while (!string.IsNullOrEmpty(nextUrl))
            {
                ...
                // FunctionPoint gives relative URLs like "/estimates?page=2"
        ...
        .WithName("GetEstimatesAllPages")
        .WithOpenApi(op =>
        {
            op.Summary = "Fetch estimates from FunctionPoint with optional filters.";
            op.Description = "Keeps calling hydra:next until no more pages remain.";
            return op;
        });
    }
}
